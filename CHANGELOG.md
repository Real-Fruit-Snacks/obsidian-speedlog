...
### Fixed
- The Cancel button painted red text on a red background (1.02:1). It now keeps the ordinary button background and colours only its label with `--text-error`; a red fill relies on the theme's `--background-modifier-error` and `--text-on-accent` agreeing, which they often don't. The label is mixed slightly toward the body text colour so it clears AA in both schemes whatever red the theme uses.
